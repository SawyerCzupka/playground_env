...
class PlayGroundNavigationV1(gym.Env):
    ...
    def reset_with_goal(self, goal_str):
        ...
        if words[0] == "Grow":
            ...
            if (
                obj_to_be_grown["categories"] in ["living_thing", "plant"]
                or obj_to_be_grown["types"] in self.categories["plant"]
            ):
                obj_supply.update(dict(types="water", categories="supply"))
            else:
    ...
    def reset_scene(self, objects=None):
        ...
        for obj in self.objects:
            self.object_grasped = obj.update_state(
                self.agent_pos,
    ...
    def step(self, action):
        ...
        self.agent_pos = np.clip(
            self.agent_pos + action[:2] * self.agent_step_size, -1.2, 1.2
        )
        ...
        assert self.observation is not None, "Observation is None. Call reset() first."
    ...
    def render(self, goal_str="", mode="human", close=False):
        ...
        ), "Cannot render if viewer is None. Create environment with render_mode=True."
        ...
        if self.reward_screen:
            ...
            self.viewer.blit(
                goal_txt_surface, (800 + 150 - goal_txt_surface.get_width() // 2, 50)
            )
            ...
            for i_obj, object in enumerate(self.objects):
                ...
                pygame.draw.rect(
                    ...
                    (860 + int(x * 160), 252.5 + 200 * i_obj, 3, 25),
                )
        ...
        if self.first_action == False:
            ...
            speech_bubble_icon = pygame.image.load(
                self.params["img_path"] + "bubble.png"
            )
            speech_bubble_icon = pygame.transform.scale(
                speech_bubble_icon, (txt_surface.get_width() + 50, 120)
            ).convert_alpha()
        ...
        if self.viz_data_collection:
            ...
            known_goals_icon = pygame.image.load(
                self.params["img_path"] + "known_goals_box.png"
            )
            known_goals_icon = pygame.transform.scale(
                known_goals_icon, (300, 35 + 25 * len(self.known_goals_descr))
            ).convert_alpha()
            ...
            if self.SP_feedback == True:
                # SOCIAL PEER
                SP_head_icon = pygame.image.load(
                    self.params["img_path"] + "SP_head.png"
                )
                SP_head_icon = pygame.transform.scale(
                    SP_head_icon, (80, 80)
                ).convert_alpha()
                SP_x = 50
                SP_y = 700
                self.viewer.blit(SP_head_icon, (SP_x, SP_y))
                SP_txt_surface = FONT.render(
                    "You " + "g" + self.SP_goal_descr[1:], True, pygame.Color("black")
                )
                SP_bubble_icon = pygame.image.load(
                    self.params["img_path"] + "SP_bubble.png"
                )
                SP_bubble_icon = pygame.transform.scale(
                    SP_bubble_icon, (SP_txt_surface.get_width() + 50, 80)
                ).convert_alpha()
                ...
                if self.known_goals_update == True:
                    if self.SP_goal_descr not in self.known_goals_descr:
                        known_goals_icon = pygame.transform.scale(
                            known_goals_icon,
                            (300, 35 + 25 * (1 + len(self.known_goals_descr))),
                        ).convert_alpha()
                        self.viewer.blit(known_goals_icon, (50, 50))
                        self.viewer.blit(known_goals_txt, (75, 60))
                        for i, descr in enumerate(self.known_goals_descr):
                            goal_txt_surface = FONT.render(
                                descr, True, pygame.Color("black")
                            )
                            self.viewer.blit(goal_txt_surface, (100, 85 + 25 * i))

                        if len(self.known_goals_descr) > 1:
                            goal_txt_surface = FONT.render(
                                self.SP_goal_descr, True, pygame.Color("black")
                            )
                            self.viewer.blit(
                                goal_txt_surface,
                                (
                                    ...
                                    SP_y
                                    - int(
                                        self.progress_goal_move
                                        * (
                                            SP_y
                                            - 85
                                            - 25 * (len(self.known_goals_descr))
                                        )
                                    ),
                                ),
                            )
                            ...
                            self.viewer.blit(
                                ...
                                (
                                    100,
                                    SP_y
                                    - int(self.progress_goal_move * (SP_y - 100))
                                    - 15,
                                ),
                            )
